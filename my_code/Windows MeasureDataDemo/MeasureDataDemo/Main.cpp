...
extern "C" _declspec(dllexport) signed char OpenCom(unsigned long ucComNo, unsigned long baud)
{
...
char waitAck(void (*pFunction)(char x),u8 cid,u8 mid,char parameter)
{
	...
	Sleep(100);
	ReceiveData(cid, mid, ucComNo,chrBuffer);
...
DWORD WINAPI DataHandleProc(LPVOID lpParam)
{
	...
	if (dataReady == 1)
	{
		ReceiveData(0x06, 0x81, ucComNo, chrBuffer);
	}
...
extern "C" _declspec(dllexport) void useReceiveData(u8 cid, u8 mid, unsigned long ucComNo, unsigned char *chrBuffer)
{
	ReceiveData(cid, mid,ucComNo,chrBuffer);
}

void ReceiveData(u8 cid,u8 mid, unsigned long ucComNo, unsigned char* chrBuffer)
{
	int index = 0;
	int frameStart = 0;
	int ind = 0;
	...
	unsigned char * pBufferStart = (unsigned char*)chrBuffer;
	unsigned char * pBufferEnd = pBufferStart + RING_BUFFER_SIZE;
	unsigned char * pCustomer = pBufferStart;
	unsigned char * pProducer = pBufferStart;
