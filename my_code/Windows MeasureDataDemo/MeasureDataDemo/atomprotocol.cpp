...
extern "C" _declspec(dllexport) u8  Atom_BCC(u8 *addr, u16 len)
{
...
/*-----------------------send command--------------------------*/
extern "C" _declspec(dllexport) int  AtomCmd_Compose_Send(u8 MADDR, u8 ClassID, u8 msgID, u8* payloadBuffer, u16 PayloadLen)
{
	int index = 0;
	int ret = 0;
	u8 crc;

	unsigned char *pBuf = (unsigned char*)malloc(PayloadLen);
	...
	SendUARTMessageLength(7, (const char *)pBuf, index);

	return ret;
}

extern "C" _declspec(dllexport) int  AtomSaveToRingBuffer(u32 usLength, u8* chrBuffer, u8** pProducer, u8* ringBuffer, u32 ringBufferLen)
{
	int chrIndex = 0;
	int ret = 0;
...
/*
* 寻找
*/extern "C" _declspec(dllexport) int AtomCmd_SearchingFrame(u8 **pCurrent, u32 rBufferLen, u32* punProcessedLen, u32 *pPacketLength)
{
