...
int Receive_count;
extern "C" _declspec(dllexport) unsigned short CollectUARTData(const unsigned long ulCOMNo,char chrUARTBufferOutput[])
{
...
extern "C" _declspec(dllexport) signed char useSendUARTMessageLength(const unsigned long ulChannelNo, const char chrSendBuffer[], const unsigned short usLen)
{
	return SendUARTMessageLength(ulChannelNo, chrSendBuffer, usLen);
}
