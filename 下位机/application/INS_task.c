...
//这些方法的定义应该实在大疆封装的一个lib下，窒息。。。
void AHRS_init(fp32 quat[4],const fp32 accel[3],const fp32 mag[3]);
bool_t AHRS_update(fp32 quat[4], const fp32 time, const fp32 gyro[3], const fp32 accel[3], const fp32 mag[3]);
...
//手动修正四元数，欧拉角所需变量和方法，失败，未用到
fp32 corrected_quat[4] = {0,0,0,0};
fp32 correccted_angle[3] = {0,0,0};
void angle_correction(fp32* raw_angle, fp32* correccted_angle);
void quat_correction(fp32* quat, fp32* corrected_quat);
...
void INS_task(void const *pvParameters)
{
    ...
    while (1)
    {
        ...
        // 手动修正四元数并获得欧拉角，失败。
				//quat_correction(INS_quat, corrected_quat);
				//get_angle(corrected_quat, correccted_angle + INS_YAW_ADDRESS_OFFSET, correccted_angle + INS_PITCH_ADDRESS_OFFSET, correccted_angle + INS_ROLL_ADDRESS_OFFSET);
		    //angle_correction(INS_angle, correccted_angle);
...
/**
 * @brief                   尝试手动矫正四元数，但是可能是我没有掌握正确数学建模的方法，结果还是有巨大误差
 *                          个人认为还是得重新撸一个数据融合算法，这个方法没有用到
 * @param  quat             四元数数组
 * @param  corrected_quat   修正后的四元数数组
 */
void quat_correction(fp32* quat, fp32* corrected_quat)
{
	static fp32 last_quat[4] = {0,0,0,0};
	static fp32 this_quat[4] = {0,0,0,0};
	static fp32 delta_quat[4] = {0,0,0,0};
	static fp32 correction[4] = {0,0,0,0};
	static fp32 k = 2.5;
//	memcpy(this_quat, last_quat , sizeof(fp32)*4);
//	memcpy(quat, this_quat , sizeof(fp32)*4);
	for(int i = 0 ; i < 4 ; i++)
	{
		last_quat[i] = this_quat[i];
	}
	for(int i = 0 ; i < 4 ; i++)
	{
		this_quat[i] = quat[i];
	...
	for(int i = 0 ; i < 4 ; i++)
	{
		delta_quat[i] = this_quat[i] - last_quat[i];
	}
	for(int i = 0 ; i < 4 ; i++)
	{
		correction[i] = k * delta_quat[i];
	}
	for(int i = 0 ; i < 4 ; i++)
	{
		corrected_quat[i] += correction[i];
	}
}
/**
 * @brief                   尝试手动矫正欧拉角，失败，未用到
 * @param  raw_angle        原yaw角度
 * @param  correccted_angle 修正yaw角度
 */
#define k 0.001f
 void angle_correction(fp32* raw_angle, fp32* correccted_angle)
 {
   static fp32 last_raw = 0;
   static fp32 this_raw = 0;
   static fp32 delta_angle = 0;
   static fp32 corrected_delta_angle = 0;
   //static fp32 k = 0.001;
   last_raw = this_raw;
   this_raw = *raw_angle;
 	if(this_raw < 0)
 	{
 		this_raw = 2 * PI + this_raw;
 	}
   delta_angle = this_raw - last_raw;
   corrected_delta_angle = delta_angle + k * delta_angle;
   *correccted_angle += corrected_delta_angle;
 }
/**
 * @brief 这一坨东西是我在C板的例程中找到的数据融合算法，但是误差大的离谱，所以没有用到
 */
//void AHRS_init(fp32 quat[4], const fp32 accel[3], const fp32 mag[3])
//{
//    quat[0] = 1.0f;
