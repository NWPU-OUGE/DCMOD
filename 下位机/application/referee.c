...
void referee_data_solve(uint8_t *frame)
{
    ...
    memcpy(&referee_receive_header, frame, sizeof(frame_header_struct_t));      //第一个数据为包头，长度5字节

    index += sizeof(frame_header_struct_t);

    memcpy(&cmd_id, frame + index, sizeof(uint16_t));       //第二组数据为cmd_id（应该是数据内容id），长度2字节
    ...
    switch (cmd_id)
    {
        ...
        {
            memcpy(&game_state, frame + index, sizeof(ext_game_state_t));       //4字节
        }
        break;
        case GAME_RESULT_CMD_ID:
        {
            memcpy(&game_result, frame + index, sizeof(game_result));       //1字节   游戏结果信息
        }
        break;
        case GAME_ROBOT_HP_CMD_ID:
        {
            memcpy(&game_robot_HP_t, frame + index, sizeof(ext_game_robot_HP_t));       //28字节  机器人血量信息
        }
        break;
        ...
        {
            memcpy(&field_event, frame + index, sizeof(field_event));       //4字节
        }
        break;
        case SUPPLY_PROJECTILE_ACTION_CMD_ID:
        {
            memcpy(&supply_projectile_action_t, frame + index, sizeof(supply_projectile_action_t));     //4字节
        }
        break;
