...
//留意，这个底盘行为模式变量
catch_behaviour_e catch_behaviour_mode = CATCH_NO_MOVE;
...
/**
  ...
  * @retval         none
  */
void catch_behaviour_mode_set(catch_move_t *catch_move_mode)
{
    ...
    //remote control  set catch behaviour mode
    //遥控器设置模式
    if (switch_is_up(catch_move_mode->catch_RC->rc.s[CATCH_MODE_CHANNEL]))
    {
        //catch_behaviour_mode = CATCH_FOLLOW_INVOLUTION;
		//catch_behaviour_mode = CATCH_FOLLOW_ANGLE;
        catch_behaviour_mode=CATCH_FOLLOW_CHASSIS_AND_ANGLE;
    }
    else if (switch_is_mid(catch_move_mode->catch_RC->rc.s[CATCH_MODE_CHANNEL]))
    {
        //can change to CATCH_ZERO_FORCE,CATCH_NO_MOVE,CATCH_INFANTRY_FOLLOW_GIMBAL_YAW,
        //CATCH_ENGINEER_FOLLOW_CATCH_YAW,CATCH_NO_FOLLOW_YAW,CATCH_OPEN
        
        //catch_behaviour_mode =  CATCH_FOLLOW_CHASSIS;
        catch_behaviour_mode=CATCH_FOLLOW_CHASSIS_AND_ANGLE;
        //catch_behaviour_mode = CATCH_FOLLOW_CHASSIS;
    }
    else if (switch_is_down(catch_move_mode->catch_RC->rc.s[CATCH_MODE_CHANNEL]))
    {
        catch_behaviour_mode = CATCH_NO_MOVE;
    }
    ...
    //accord to beheviour mode, choose catch control mode
    //根据行为模式选择一个持球控制模式
    
    if (catch_behaviour_mode == CATCH_NO_MOVE)//不动如山模式
    {
        catch_move_mode->catch_mode = CATCH_VECTOR_NO_FOLLOW_YAW; 
    }
    else if(catch_behaviour_mode == CATCH_FOLLOW_CHASSIS)//持球跟随底盘模式
    {
        catch_move_mode->catch_mode = CATCH_VECTOR_FOLLOW_CHASSIS;
    }
    else if(catch_behaviour_mode == CATCH_FOLLOW_ANGLE)//持球跟随角度传感器角度环
    {
        catch_move_mode->catch_mode = CATCH_VECTOR_FOLLOW_ANGLE;
    }
    else if(catch_behaviour_mode == CATCH_FOLLOW_CHASSIS_AND_ANGLE)//持球同时又底盘和角度传感器角度环决定
    {
        catch_move_mode->catch_mode = CATCH_VECTOR_FOLLOW_CHASSIS_AND_ANGLE;
    }
    else if(catch_behaviour_mode == CATCH_FOLLOW_INVOLUTION)//持续性内卷分量
    {
        catch_move_mode->catch_mode = CATCH_VECTOR_FOLLOW_INVOLUTION;
    }
}
